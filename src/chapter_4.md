...
Поддержка различных файлов в TyranoBuilder часто зависит от используемой платформы. Чтобы достичь максимальной 
переносимости игры рекомендуется использовать форматы файлов для изображения: _.jpg_, _.png_, _.gif_, для видео: _.ogv_,
_.webm_, для аудио: _.ogg_, _.mpa_.

### Несколько слов о Настройках игры

Для того чтобы изменить параметры игры достаточно нажать иконку с изображением шестерёнки в верхнем правом углу 
приложения, после чего откроется окно _"Game Settings (Настройки Игры)"_. Вкладка _"General (Общие)"_ позволяет 
изменить название игры, а на вкладке _"Keyboard & Mouse (Клавиатура и мышь)"_ можно настроить функциональные клавиши 
или добавить новые, например, такие как _"Quick Load/Save (Быстрая(ое) загрузка/сохранение)"_ или _"Skip Dialogue 
(Пропустить диалог)"_. На этой же вкладке есть возможность настроить функции кнопок мыши и функции жестов для 
смартфона. Если вам нужно изменить расширение игры, то можно воспользоваться вкладкой _"Screen size (Размер экрана)"_. 
Так же в процессе разработки игры вам может понадобиться изменить параметры шрифта, то для этого существует вкладка 
_"Font Style (Стиль шрифта)"_.

### Скрипты в TyranoBuilder

TyranoBuilder предлагает два компонента для написания скриптов это _iScript_ и _TyranoScript_.

_TyranoScript_ – это текстовая версия TyranoBuilder, то есть вместо использования перетаскиваемых компонентов, вы 
вводите команды вручную. 

Если нужно расширить функциональность TyranoBuilder используйте _iScript_, основан на JavaScript, которые отличаются 
между собой способом работы с переменными. 

Чтобы добавить сценарии в TyranoBuilder просто перетащите компонент iScript или TyranoScript на сцену проекта.

Сперва давайте познакомимся с TyranoScript:

``` TyranoScript
Эта строка отобразиться как диалог.
...
В данном листинге показано, как работают теги в TyranoScript, которые оборачиваются в квадратные скобки. В указанном 
примере мы познакомились с основными тегами для управления отображения диалога в игре.

Теперь познакомимся с iScript.
...
В этом простом примере показано, как iScript работает с внутриигровыми переменными используя глобальную переменную _f_, 
вместо привычно _var_, который используется в JavaScript.

### Live2D

Технология Live2D, разработанная Тецуя Накаджо, позволяет создавать 2D анимированных персонажей без использования 
покадровой анимации и 3D-моделей. Для этого создается набор двухмерных изображений глаз, рук, и других частей тела, 
затем добавляется скелетная структура, при помощи которого выполняется анимация на основе заранее определенных поз и 
движений. 

Хотя TyranoBuilder не имеет инструментария для создания Live2D персонажей, но он поддерживает эту технологию. 
Следовательно, для создания Live2D персонажей придётся использовать стороннее программное обеспечение.

Теперь давайте разберёмся как же добавить Live2D персонажа в TyranoBuilder. Для это нужно активировать технологию 
Live2D. Выбираем пункт меню _"Project (Проект)\ Add-In Components (Интегрировать компоненты)"_. В появившемся окне 
ставим галочку на против _"Live2D Components (Live2D Компоненты)"_, и подтверждаем интеграцию нажав кнопку _"ОК"_. 
В результате вы получите уведомление об успешном добавлении Live2D компонентов к вашему проекту.

Теперь импортируем Live2D персонажа в игру. На верхней панели инструментов находим _Live2D_, нажимаем, откроется окно 
_Live2D Model Settings (Live2D Настройки моделей)_, где импортируем персонажа в проект нажав кнопку 
_Add a Live2D Model (Добавить Live2D модель)_ (см. рис. 4-7). Далее перетягиваете компонент 
_"Load Model (Загрузить модель)"_ и в параметрах выбираете нужную вам модель персонажа. После чего добавляете в сцену 
компонент _"Add Live2D (Добавить Live2D)"_ где указываете в параметрах ранее загруженную модель персонажа. Чтобы 
указать расположение персонажа на сцене воспользуйтесь инструментом _Positioning Tool (Инструмент позиционирования)_.

![Рисунок 4-7. Выбор анимации действия Live2D персонажа](./img/chapter_4/figure_4_7.png)

__Рисунок 4-7.__ Выбор анимации действия Live2D персонажа.

Запустив игру, вы увидите, как Live2D персонажи благодаря реалистичной анимации оживают в игре. Но если вам нужно 
сменить анимацию действия персонажа, то перетяните под добавленный компонент _Add Live2D_ компонент 
_Live2D Motion (Live2D Движение)_. В параметрах в поле Model указываем модель персонажа, а в поле 
_Select a Motion (Выбор движения)_ анимацию действия. Так же для этих целей можно воспользоваться 
_Motion Selection Tool (Инструментом выбора движения)_, который позволяет выбрать анимацию действия с предварительным 
просмотром. 

После выполненных действий, запускаем игру и наблюдаем как Live2D персонаж, находясь в анимации бездействия переходит 
в выбранную анимацию действия.

## Заключение

Прочитав эту главу книги, вы познакомились с Ren’Py и должны уже знать:

* Как создать новый проект и как использовать лаунчер
* Основные элементы сценариев
* Как Ren’Py работает с отступами и текстовыми блоками

Относительно TyranoBuilder, вы должны знать:

* Как создать новый проект
* Как создавать сцены и переходы между ними
* Изучили интерфейс TyranoBuilder и его основные компоненты 
* Как импортировать Live2D персонажей в проект

А о Twine вы должны знать:

* Как организованы комнаты/локации, переходы, и как реализовано взаимодействие с пользователем
* Как использовать мультимедиа при помощи HTML и встроенных макросов
* Познакомились с каскадными таблицами стилей (CSS) в контексте Twine

В следующей главе мы более подробно познакомимся с разработкой игр при помощи рассматриваемого программного обеспечения.