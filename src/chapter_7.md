...
И вот наши герои в зале резиденции президента. Теперь пришло время познакомиться ещё с одним элементом Harlowe – это 
_prompt_. Он позволит запросить у игрока пароль. Если он окажется правильным, то игрока перебросит в параграф 
_"Готово"_, иначе отобразится мигающее сообщение об ошибке с предложением повторить ввод снова. Повторная попытка – это 
простая перезагрузка страницы, которая произойдёт, когда параграф будет ссылаться сам на себя.
...
Теперь познакомимся с макросом _put:_. Это просто другая интерпретация знакомого макроса _set:_, только с той разницей, 
что для _put:_ используется другой синтаксис, но в результате они оба объявляют и изменяют значение переменной. То есть 
вместо _(set: $password ...)_ воспользуемся _(put: ... into $password)_.

Далее создадим параграф _"Готово"_ и прикрепим к нему тег _finale_.
...
А для финального параграфа _"бежать"_ напишем следующий код:
...
Неплохо бы в игру добавить звук, но, к сожалению, формат истории Harlowe не имеет таких макросов. Это можно реализовать 
с помощью HTML или JavaScript. В параграфе _"бежать"_ воспользуемся элементом _\<audio\>_, который позволяет воспроизвести 
звук как автоматически, так и при помощи элементов управления.

Вот и завершили третью часть обучающей игры _"Август возвращается"_ на Twine. Хотя некоторые эпизоды истории остаются 
ещё не рассказанными, но с основами создания игры на Harlowe мы уже познакомились.
